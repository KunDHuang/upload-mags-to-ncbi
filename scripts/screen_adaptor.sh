...
help_page () {
    ...
    echo " Options:"
    echo ""
    echo " -mags_dir    STR        Absolute path to the folder containing MAGs."
    echo " -opt_dir     STR        Absolute path to the output folder."
    echo " -mem         INT        Memory allocation (default: 12 GB)"
    echo " -cpu         INT        CPUs per sample (default: 5)"
    echo " -log_dir     STR        Path for logs (default: /vol/cluster-data/khuang/slurm_logs)"
    echo " -time        INT        Walltime in hours (default: 24)"
    echo ""
